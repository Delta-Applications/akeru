...
(function(exports) {
  ...
  var SettingsCache = {
    ...
    observe: function observe(name, defaultValue, callback, observeOnly) {
      if (_settingsCache && !observeOnly) {
        // eslint-disable-next-line
        callback(typeof _settingsCache[name] !== 'undefined'
          ? _settingsCache[name] : defaultValue, name);
      }

      const settingChanged = function settingChanged(evt) {
        callback(evt.settingValue, evt.settingName);
      };
      mozSettings.addObserver(name, settingChanged);
      observers.push({
        name,
        callback,
    ...
    unObserve: function unObserve(name, callback) {
      observers.forEach((value, index) => {
        if (value.name === name && value.callback === callback) {
          mozSettings.removeObserver(name, value.observer);
  ...
  exports.SettingsDBCache = SettingsCache;
