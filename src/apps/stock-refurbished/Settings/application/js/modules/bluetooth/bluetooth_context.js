 ...
 */
define(['require','modules/bluetooth/bluetooth_adapter_manager','modules/bluetooth/bluetooth_device','modules/bluetooth/bluetooth_connection_manager','modules/mvvm/observable','modules/mvvm/observable_array'],function(require) {
  ...
  var BluetoothContext = {
    ...
    function btc__onAdapterAttributeChanged(adapter, evt) {
      for (var i in evt.attrs) {
        ...
        switch (evt.attrs[i]) {
          ...
          case 'discovering':
            this.discovering = adapter.discovering;
            if (!this.discovering && this._scanTimeoutId) {
    ...
    startDiscovery: function btc_startDiscovery() {
      ...
      this._scanTimeoutId = setTimeout(() => { this.stopDiscovery() }, SCANTIMEOUT);
    ...
    resetPbmapSetting: function btc_resetPbampSetting(address) {

      settings.createLock().get(BLUETOOTH_PBAP).then((result) => {
        let value = result[BLUETOOTH_PBAP];
      ...
      settings.createLock().get(BLUETOOTH_MAP).then((result) => {
        let value = result[BLUETOOTH_MAP];
