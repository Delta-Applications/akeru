 ...
 */
define(['require','modules/bluetooth/bluetooth_adapter_manager'],function(require) {
    ...
    _updateConnectionStatus: function btcm___updateConnectionStatus(options) {
      ...
      this._updateConnectedDevices(options).then((connectedDevicesInfo) => {
        ...
        if (options.connected) {
          ...
          /*
           * If the device connection is manual connection,
           * don't fire 'disconnected' event'
           * Maybe the a2dp is connecting at the time.
           */
          if (!this._manualConnection) {
            // If there is no profile connected,
