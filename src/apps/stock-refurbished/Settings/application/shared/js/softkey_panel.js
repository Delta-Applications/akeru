...
var SoftkeyPanel = function (actions, optionMenuCallback, area) {
    ...
    this.keyDownHandler = (function (e) {
        ...
        if ((e.key === 'Enter' || e.key === 'Accept') && this.menuVisible && this.menu) {
        ...
        if (typeof NavigationMap !== 'undefined' && NavigationMap.currentActivatedLength > 0) {
            return;
        }
        switch (e.key) {
            case 'BrowserSearch':
    ...
    function setSoftkeys() {
        function getSkHtml(item) {
            ...
            try {
                return item.icon ? (`<span data-name="${name}">${item.iconMixedText ? name : ''}</span>
              <span data-icon="${item.icon}"></span>`) : name;
            } catch (e) {
                console.warn(e);
                return name;
            }
        ...
        if (this.lskIndex !== -1) {
            updateSoftKeys(this.actions, this.handlers, this.buttonLsk, this.lskIndex);
        } else {
        ...
        if (this.cskIndex !== -1) {
            updateSoftKeys(this.actions, this.handlers, this.buttonCsk, this.cskIndex);
        } else {
        ...
        if (this.rskIndex !== -1) {
            updateSoftKeys(this.actions, this.handlers, this.buttonRsk, this.rskIndex);
    ...
    function openMenu() {
        ...
        if (typeof OptionMenu == 'undefined') {
            LazyLoader.load(['/shared/js/option_menu.js', '/shared/style/action_menu.css', '/shared/style/option_menu.css'], () => {
...
function updateNextSoftkeyPanelKeysInfo() {
    const softkeyPanels = document.querySelectorAll('#softkeyPanel.visible.focused');
...
function modifyAllString() {
    ...
    const softkeyPanels = document.querySelectorAll('#softkeyPanel.visible.focused');
    if (softkeyPanels.length && softkeyPanels[softkeyPanels.length - 1] === this.form) {
