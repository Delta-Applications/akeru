  ...
  */
  ...
  function searchForLinks(type, string) {
    ...
    while (match = regexp.exec(string)) {
      ...
      // According to the International numbering plan, the number max length is 15.
      // Please refer to https://en.wikipedia.org/wiki/Telephone_numbering_plan.
      if (type === 'phone' && match[0].length > 15) {
        continue;
      }
    ...
    searchAndLinkClickableData:
     function lh_searchAndLinkClickableData(inputText, mode) {
      ...
      if (mode.hasOwnProperty('email') && !Settings.emailAppInstalled) {
        linkSpecs.forEach((value, index) => {
          value['type'] === 'email' && linkSpecs.splice(index, 1);
        });
      }
