...
(function(exports) {
  ...
  function getPhotoHeader(contact, contactName) {
    if(contact && contact.photo && contact.photo.length) {
      var contactImage = getThumbnail(contact);
  ...
  function getDefaultImage(contact, contactName) {
    var pictureContainer = document.createElement("span");
    ...
    var posVertical = ['top','center','bottom'];
    var posHorizontal = ['left','center','right'];
    var position =
  ...
  function getPhotoHeaderByImg (contactImage) {
    var photoView = document.createElement("span");
    ...
    try {
      ...
      console.warn('Failed to create contact picture : ' +
