...
(function(){var loader=LazyLoader;loader.load("js/navigation_map.js",function(){NavigationMap.init();});window.addEventListener("keydown",function(evt){handleKeydown(evt);});window.addEventListener("menuEvent",function(event){NavigationMap&&(NavigationMap.optionMenuVisible=event.detail.menuVisible);});function handleKeydown(evt){var el=evt.target,bestElementToFocus;if(NavigationMap&&(NavigationMap.currentActivatedLength>0||NavigationMap.lockNavigation)){return;}
