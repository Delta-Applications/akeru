...
(function(exports) {
  ...
  let VCARD_SKIP_FIELD = ['fb_profile_photo'];
  let VCARD_VERSION = '3.0';
  let HEADER = 'BEGIN:VCARD' + CRLF + 'VERSION:' + VCARD_VERSION + CRLF;
  let FOOTER = 'END:VCARD' + CRLF;
  ...
  function fromContactField(sourceField, vcardField) {
    ...
    return sourceField.map(function(field) {
      ...
      if (Array.isArray(field.type)) {
        let fieldType = field.type.map(function(aType) {
          let out = '';
          if (aType) {
  ...
  function ContactToVcardBlob(contacts, callback, options) {
    let targetType = options && options.type || 'text/vcard';
  ...
  function ContactToVcard(contacts, append, success, batchSize, skipPhoto) {
    ...
    function processContact(ct) {
      ...
      if (ct.xirmc) {
        allFields.push('X-IRMC-CALL-DATETIME;' + ct.xirmc);
      }
      ...
      let adrs = fromContactField(ct.adr, 'ADR');
      ...
      */
      if (
        ...
        let photoMeta = ['PHOTO', 'ENCODING=b'];
        let blob = ct.photo[0];
  ...
  function getVcardFilename(theContact) {
    ...
    let givenName = Array.isArray(theContact.givenName) &&
                                                      theContact.givenName[0];
    let familyName = Array.isArray(theContact.familyName) &&
