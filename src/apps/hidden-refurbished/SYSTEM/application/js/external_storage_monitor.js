  ...
  ExternalStorageMonitor.prototype = {
    ...
    start: function() {
      ...
      if (!this._storage) {
        ...
        this.showStorageNotificationIfNecessary(sdcardDetect);
      } else {
        this._storage.available().then((result) => {
          ...
          if (sdcardDetect) {
            var valueHandler = () => {
              navigator.mozSettings.removeObserver(DEFAULT_MEDIA_VOLUME_KEY,
                valueHandler);
            ...
            navigator.mozSettings.addObserver(DEFAULT_MEDIA_VOLUME_KEY,
              valueHandler);
          }
        });
      }
    ...
    showStorageNotificationIfNecessary: function(sdcardDetect) {
      ...
      if (!lastDetectedTime && sdcardDetect) {
        ...
        this.showNotification(sdcardDetect);
    ...
    showNotification: function(sdcardDetect) {
      var _ = navigator.mozL10n.get;
      var body;
      if (sdcardDetect) {
        body = _('sdcard-detected-title');
      } else {
        body = _('no-sdcard-title');
      }
      var title = _('storage');
